...
for year in range(2010, 2024):
    ...
    for page in range(1, 6):
        # Open PubMed
        driver.get(f"https://pubmed.ncbi.nlm.nih.gov/?term={search_query}&filter=years.{year}-{year}&page={page}")
        ...
        for result in results:
            ...
            abstract = article_soup.find("div", {"class": "abstract-content"}).text
            ...
            year_data.append({"Title": title, "PMID": pmid, "Year": year, "Authors": authors, "Link": link, "Model": model, "Downloadability": downloadability, "Abstract": abstract, "Dataset Size": data_set, "Top Performing Model": top_model})
...
excel_writer = pd.ExcelWriter("pubmed.xlsx")
...
print(f"Scraped data saved to pubmed.xlsx with separate sheets for each year.")
